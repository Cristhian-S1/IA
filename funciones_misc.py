...
def reconstruir(solucion_plana, tamanio_tablero):
    # Convierte la lista plana de MiniKanren en cuadrícula 2D (lista de listas).
    tablero = []
...
def verificar_solucion(tablero):
    # Valida que todos los bordes adyacentes coincidan; imprime los errores encontrados.
    if tablero is None:
        return False
...
def imprimir_puzzle(tablero, titulo="TetraVex"):
    # Muestra el tablero en consola en formato legible (izq der arr abj) por pieza.
    if tablero is None:
        print(f"\n{titulo}: Sin solucion!")
        return
...
def tetravex_aleatorio(tamanio_tablero, valor_maximo_ficha=9):
    # Genera un puzzle aleatorio con solución garantizada: construye bordes coherentes y luego baraja las piezas.
...
def leer_puzzle_manual(tamanio_tablero):
    # Lee N*N piezas desde teclado en formato "izq der arr abj" y las devuelve como cuadrícula 2D.
